function instructions(input) {
    ...
    input.split('\n').forEach(instruction => (() => {
        let [variable, operation, value,, conditionVariable, condition, conditionValue] = instruction.split(' ')
        vars[variable] = vars[variable] || 0
        vars[conditionVariable] = vars[conditionVariable] || 0
        operation = operation === 'dec' ? '-=' : '+='
        eval(`if (vars.${conditionVariable} ${condition} ${conditionValue}) vars.${variable} ${operation} ${value}`)

        const highestCurrentValue = Math.max(...Object.values(vars))
        
        if (highestCurrentValue > highestValue) {
            highestValue = highestCurrentValue
        }
    })())
...
console.assert(instructions(`b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10`) === 10)

console.log(instructions(`smi inc 781 if epx > -2
