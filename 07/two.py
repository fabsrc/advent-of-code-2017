...
def balance(input):
    ...
    for line in input.splitlines():
        name, weight, children = re.match("(\w+)\s\((\d+)\)(?:\s->\s(.*))?", line).groups()

        discs[name] = discs.get(name, {})
        discs[name]["name"] = name
        discs[name]["weight"] = int(weight)

        if children:
            children = children.split(", ")

            if "children" in discs[name]:
                discs[name]["children"] += children
            else:
                discs[name]["children"] = children

            for child in children:
                discs[child] = discs.get(child, {})
