...
def balance(input):
    ...
    for line in input.splitlines():
        ...
        if name in list:
            list[name]["weight"] = weight
        else:
            list[name] = { "weight": weight }

        if children:
            children = children.split(", ")
            if "children" in list[name]:
                list[name]["children"] += children
            else:
                list[name]["children"] = children
            for child in children:
                if child in list:
                    list[child]["parent"] = name
                else:
                    list[child] = { "parent": name }

    bottom = [name for name in list if "parent" not in list[name]][0]
    return bottom
