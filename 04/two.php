...
function passphrase($input) {
    ...
    foreach($lines as $line) {
        ...
        $analyzed_words = array_map(function($word) {
            return serialize(count_chars($word, 1));
        }, $words);
        $unique_analyzed_words = array_unique($analyzed_words);

        if ($analyzed_words === $unique_analyzed_words) {
            $count++;
        }
    }
...
assert(passphrase("abcde fghij") === 1);
assert(passphrase("abcde xyz ecdab") === 0);
assert(passphrase("a ab abc abd abf abj") === 1);
assert(passphrase("iiii oiii ooii oooi oooo") === 1);
assert(passphrase("oiii ioii iioi iiio") === 0);
echo(passphrase(
